...
namespace FLEX.Web.Core
{
   ...
   internal sealed class ViewStateStorageSettings : ICloneable
   {
      ...
      {
         get { return persistenceHandler; }
         set { persistenceHandler = value; }
      }
      ...
      public ViewStateStorageSettings(XmlNode node)
      {
         ...
         XmlAttribute handlerName = node.Attributes["PersistenceHandler"];
         if (handlerName != null)
            this.persistenceHandler = handlerName.Value;


         XmlAttribute storagePath = node.Attributes["StoragePath"];
         if (storagePath != null)
            this._storagePath = storagePath.Value;


         XmlAttribute connString = node.Attributes["ConnectionString"];
         if (connString != null)
            this._connectionString = connString.Value;

         XmlAttribute tableName = node.Attributes["TableName"];
         if (tableName != null)
            this._tableName = tableName.Value;


         XmlAttribute storageMethod = node.Attributes["StorageMethod"];
         ...
         XmlAttribute compressed = node.Attributes["Compressed"];
         if (compressed != null)
         {
            this._compressed = ( String.Compare(compressed.Value, bool.TrueString, true) == 0 );
         }

         XmlAttribute behavior = node.Attributes["RequestBehavior"];
         if (behavior != null)
         {
            try 
            {
         ...
         XmlAttribute viewstatefilesMaxAge = node.Attributes["ViewStateFilesMaxAge"];
         ...
         XmlAttribute viewstateCleanupInterval = node.Attributes["ViewStateCleanupInterval"];
      ...
      {
         ViewStateStorageSettings settings = (ViewStateStorageSettings)System.Configuration.ConfigurationManager.GetSection("Flesk.NET/ViewStateOptimizer");
         if(settings == null)
      ...
      {
         ViewStateStorageSettings ret = new ViewStateStorageSettings();
