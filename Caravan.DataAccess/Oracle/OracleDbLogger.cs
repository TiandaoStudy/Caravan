...
namespace FLEX.DataAccess.Oracle
{
    public sealed class OracleDbLogger : DbLoggerBase, IDbLogger
    {
        ...
        public IEnumerable<LogEntry> RetrieveCurrentApplicationLogs()
        {
            ...
            using (var connection = QueryExecutor.Instance.OpenConnection())
            {
                return connection.Query<LogEntry>(query, new {Common.Configuration.Instance.ApplicationName});
            }
        }

        public DataTable RetrieveAllLogsTable()
        {
            return RetrieveAllLogs().ToDataTable();
        }

        public DataTable RetrieveCurrentApplicationLogsTable()
        {
